...
function workLoop(deadline) {
  ...
  while (nextFiberReconcileWork && !showYield) {
    nextFiberReconcileWork = performNextWork(nextFiberReconcileWork); // deadline.timeRemaining()：单位ms，返回当前空闲期间剩余的估计毫秒数。

    showYield = deadline.timeRemaining() < 1;
  }

  if (!nextFiberReconcileWork && wipRoot) commitRoot(); // requestIdleCallback:将在浏览器空闲时期被调用

  requestIdleCallback(workLoop);
}

requestIdleCallback(workLoop); //###### schedule 
//############ reconcile
...
function reconcileChildren(wipFiber, elements) {
  let index = 0;
  let prevSibling = null; // 如果wipFiber有children
...
function commitWork(fiber) {
  ...
  const domParent = domParentFiber.dom; // 暂时不做 diff 和删除修改，所以这里的 effectTag 都是 placement，也就是新增元素
